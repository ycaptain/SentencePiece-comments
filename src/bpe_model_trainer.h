...
class Trainer : public TrainerInterface {
 ...
 private:
  ...
  // 将 Position 编码成 64 位无符号整数
  // 编码后的值结构：前 32 位表示 sentence id
  // 中间 16 位表示 left symbol index
  // 后 16 位表示 right symbol index
  ...
  // 从指纹获得对应的一元符号
  // 返回值如果不在 symbols_cache_ 中则会被尝试插入 symbols_cache_，如果已存在则直接返回
  ...
  // 若指定的句子和相邻索引所代表的符号与指定的符号 best 不同
  // 则重设句子中的那个符号的出现频数为 0
  // 参数：
  ...
  // 复制出现频数在前 5% 的 symbols_cache_ 中的符号来更新 active_symbols_
  ...
  // 句子集 symbols[sid] 代表一个句子
  // symbols[sid][index] 存储着一个句 id 为 sid 的句子的 index 位置上的符号
